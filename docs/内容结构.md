...
从内容的DOM结构上进行划分，主要用于插入时的判断依据。

- `LROW`: `<li>` 列表行。
- `DROW`: `<dt>`, `<dd>` 定义列表行。
- `TROW`: `<tr>` 表格行。
- `CONITEM`: `<p>`, `<dl>`, `<ol>`, `<figure>`... 内容条目。
- `HEADING`: `<h2>` ~ `<h6>`, `<caption>`, `<figcaption>`, `<summary>` 标题。
- `SECHEAD`: `<header>` 导言。
- `SECT`: `<section>` 片区。
- `SUMMARY`: `<header role="summary">` 摘要（文章开头）
- `INLINE`: `<strong>`, `<em>`, `<mark>`, `<meter>`... 行内（内联）单元。
- `CONBOX`: `<section role="content">` 内容容器。
- `ROWBOX`: `<ol>`, `<ul>,` `<dl>,` `<tbody>/<thead>/<tfoot>` 行容器。
- `INLBOX`: `<p>`, `<li>`, `<h2>`... 内联单元容器。
- `XBOX`: 通用父容器。可插入非容器类任意命名类型的单元，用于粘贴目标父容器确认（会创建其标准子单元）。

> **注：**<br>
> 插入目标位置需要有明确的类型定义：命名类型或（通用）容器类型。<br>
> 未定义类型的源数据单元不可整体插入到目标位置，但可以提取其文本内容（如果可能）。<br>

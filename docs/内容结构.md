...
<section role="s1"></section>       <!-- 片区：S1 /header, h3, s2, footer -->
<section role="s2"></section>       <!-- 片区：S2 /header, h4, s3, footer -->
<section role="s3"></section>       <!-- 片区：S3 /header, h5, s4, footer -->
<section role="s4"></section>       <!-- 片区：S4 /header, h6, s5, footer -->
<section role="s5"></section>       <!-- 片区：S5 /header, [content]..., footer -->
<ul></ul>                           <!-- 无序列表：Ul /li/... -->
<ol></ol>                           <!-- 有序列表：Ol /li/... -->
<ol role="codes"></ol>              <!-- 代码表：Codes /li/code/b, #text -->
...
```html
<ol role="codes" data-lang="xx" data-tab="4" start="x">
    <li><code> <!-- 根封装 -->
        ...<b></b>... <!-- 代码内容，<b>着色封装 -->
    </code></li>
...
与代码表相似，包含 `data-lang` 和 `data-tab` 两个属性定义代码的特性，书写在 `<code>` 元素上。

```html
<pre role="codeblock"><!-- 容许简单换行 -->
<code data-lang="xx" data-tab="4"> ...<b>..</b>...
<!-- Tab符被自动转换为空格（如果配值非零） -->
</code><pre> <!-- 末尾应当紧跟 -->
```

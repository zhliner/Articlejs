...
可直接包含文本和内联元素的行级元素，上面的文本类内容件就是内容行。但内容行还包括不能单独作为内容的行元素，如 `<li>`、`<caption>`、`<dt>`、`<dd>` 等，它们是内容行但不是内容件。


### 内容元素

所有可以直接包含文本或其它内联元素的元素。上面的内容行是内容元素，但还有其它非行级的可直接包含文本的元素，如 `<td>`、`<strong>`、`<small>` 等，前者是表格的块单元（非行），后两者是内联元素。
...
行内单元不是独立的**内容件**，它们不能单独出现在章节片区（`<section>`）里，必须被内容行（譬如 `<p>`）封装才行。
...
只有**内容件**可以作为逻辑独立的单元出现在章节片区（`<section>`）里，它们是表达内容的一个**自我完整**的单元。
...
这是文章内容的主容器，除了文章的主标题（页面标题 `<h1>`）、提要、目录等少数几个结构外，其它所有内容件都被包含在里面。
...
代码内的高亮着色采用 `<b>` 元素封装，通过 `class` 类名区分。所有 `<b>` 元素只会处于同一层级（`<code>` 直接子元素），不存在嵌套。
...
> **注：**<br>
> 用 `<b>` 封装着色文本不但减少书写量，在没有样式渲染时，默认的粗体显示也是一种友好。

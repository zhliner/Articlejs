...
id          // b[role=id]   CSS ID选择器
cls         // b[role=cls]  CSS 类名选择器
rgb         // b[role=rgb]  CSS 颜色值 #fff #f0f0f0 rgb(214, 86, 0)
hsl         // b[role=hsl]  CSS 颜色值 hsl(24deg, 100%, 42%)
unit        // b[role=unit] CSS 单位名 px em pt
...
> 用 `<i>` 封装注释也有类似的功用，斜体的附加说明感受明确。<br>
> 用 `<s>` 封装字符串，除了 `string` 首字符相同外，默认的贯穿线或许醒目并表达非可执行的意思。<br>

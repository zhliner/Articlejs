...
=[: 讲解图 ][ `<span role="image"></span>` ][ `IMAGE /img, i:explain` ][ 36 ]
...
=[: 名称 :][ 标签 ][ 类型/结构 ][ No. ]
=[----------------------------]
=[: 按钮 ][ `<button></button>` ][ `BUTTON` ][ 39 ]
=[: 录入件 ][ `<input />` ][ `INPUT` ][ 40 ]
=[: 标签 ][ `<label></label>` ][ `LABEL` ][ 41 ]
=[: 选取项 ][ `<option></option>` ][ `OPTION` ][ 42 ]
=[: 输出件 ][ `<output></output>` ][ `OUTPUT` ][ 43 ]
=[: 进度条 ][ `<progress></progress>` ][ `PROGRESS` ][ 44 ]
=[: 选单 ][ `<select></select>` ][ `SELECT` ][ 45 ]
=[: 文本框 ][ `<textarea></textarea>` ][ `TEXTAREA` ][ 46 ]

...
- **h4**: 各行块单元的*小标题*，如：文章提要（`<header:abstract>/<h4>, <p>...`）、引用块（`<blockquote>/<h4>, <p>...`）等。
...
这里 `<li>` 作为一个简单的容器便于CSS表达，而如果使用 `<dl>/<dt>,<dd>` 结构则会显得比较笨重也不合适。并且 `<h6>` 如果作为常规的标题分级，在大多数正文中都很难见到（使用概率非常低）。
...
<figure></figure>                   <!-- 插图：FIGURE /figcaption, p, ol, ul... -->

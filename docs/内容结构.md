...
<pre role="blockcode"></pre>        <!-- 代码块：Blockcode /code/b, #text -->
```
...
每个结构性元素或内容单元的子单元类型都是有限定的。插入内容时会考虑：

1. 目标参考元素的父元素允许哪些子单元？是否需要/可以转换？
2. 目标参考元素自身作为容器允许哪些子单元，是否需要/可以转换？

**注**：转换时通常是创建一个默认的空子单元，然后提取源数据填充。


```js
//
// 首个子类型为内容类型，用于可能的内容元素创建。
// 空的子类型表示不能直接插入内容，需使用其特有的接口操作。
//
const typeMap = {
    [LROW]:     [ TEXT, INLINE ],
    [DROW]:     [ TEXT, INLINE ],
    ...
    [SECHEAD]:  [ CONITEM ]
    [SECFOOT]:  [ CONITEM ]
    [SEC1]:     [ HEADING, SECHEAD, SEC2, CONBOX, SECFOOT ]
    [SEC2]:     [ HEADING, SECHEAD, SEC3, CONBOX, SECFOOT ]
    [SEC3]:     [ HEADING, SECHEAD, SEC4, CONBOX, SECFOOT ]
    [SEC4]:     [ HEADING, SECHEAD, SEC5, CONBOX, SECFOOT ]
    [SEC5]:     [ HEADING, SECHEAD, CONBOX, SECFOOT ]
    [SUMMARY]:  [  ]
    [INLINE]:   [  ]
    [TEXT]:     [],
}

...
另外还会设置 `school.$ = student`，这样在遇到重名冲突时，可以通过 `school.$` 明确从父对象上引用。如：`school.$.name` 引用姓名（而非校名）。

> **注：**<br>
> 实际上在模板内，`school` 已经被赋值到 `$`，也只能通过 `$` 引用 `school` 的成员。因此对于被子成员名称覆盖的父级数据，就是形如 `$.$.$...` 的引用方式了。<br>
> 数组的成员可能是 `null` 或 `undefined`，当数组被展开时，成员会被赋值到 `$`。对于这两个不是对象的特殊值，它们不会被添加父域（`.$`）引用。<br>

...
// 如果最终是单个元素的单个属性对应到一个数组，则为双值切换模式。
// 例：
//      #test|@-val|...
//      数据源为 [1, 0]
// 行为：
// 对于目标元素的 data-val 特性：若原为1，则设置为0，原为0，则设置为1。
// 注记：
// 这种切换式设置是强制的，因为元素的特性/属性/样式值只应当是简单类型。
// 在tpb/tQuery中，元素上并不存储额外数据，因此这种约束是可行的。
...
- 支持多方法定义，逗号分隔。
- 多方法主要用于多数据同时设置的情况，此时每个方法通常各自需要一条数据（数组单元）。
- 如果目标唯一，多个方法及其数据应用到同一元素，否则其数据（可能是一个子数组）应用到多个元素（再次一一对应）。
- 如果最终的单个元素的单个方法的数据依然是一个数组（应该为二元），此时则为双值切换式设置。

即：最复杂的情况可能存在3层数组深度的数据定义。**注**：首先方法分组对应，然后元素分组对应（tQuery:Collector），最后双值切换模式（强制）。
...
> **注：**<br>
> 作为数据数组的成员本身也可以是一个数组，它对应某方法应用到一个集合（tQuery中数据与集合成员的一一对应）。

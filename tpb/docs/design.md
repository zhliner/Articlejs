...
行为链可视为一个指令（方法）序列，它们对一个数据栈进行操作。通常，栈内数据需要取出后再处理，它们是后续方法的操作目标。取出的数据称为**当前条目**，存放该条目的地方称为**暂存区**。暂存区是一次性使用，用过即空的逻辑。如果暂存区没有操作目标，系统会自动从栈顶取值（取出而非复制）。
...
当前条目由暂存区取值函数操作，接口：`function( n ): Value | Collector`，它在方法内被自行调用，实参 `n` 的含义：

- `0` 若暂存区有值则返回值，否则返回 `undefined`。不自动取栈。
- `x` 一个大于零的数，若暂存区有值则返回值，否则取栈顶 `x` 项：如果 `x > 1`，返回一个 `Collector`，否则返回值本身。
...
hello( $msg: Value | null ): void
// 向控制台打印消息。
// $msg为待打印的消息或当前条目（非空）的成员名，支持首字符特殊指引。
// 如果当前条目为空，$msg为字面量消息。
// 如果实参传递为 null，则取当前条目自身，且支持自动取栈。
// 目标：当前条目，条件式取栈。
//
// 注：
// - 实参首字符特殊指引（不自动取栈）。
// - 如果实参未定义或为null，打印目标本身（自动取栈）。
// - 当前条目为空时的实参字面量表达。
//
// 这是一个测试用方法。
...
env( name: String, $val?: Value | null ): void | Value
// 全局环境设置或取值。
...
vtrue( $code, ...rest ): Value
// 真值执行，否则跳过。
// $code 为函数体代码，支持首字符特殊指引。
// ...rest 为 $code 的实参序列，这在引用外部方法时很有用。
// 目标：当前条目/栈顶1项。
// 注：
// 因为缺乏操作目标，所以$xxx的指引只针对tQuery自身。

vfalse( $code, ...rest ): Value
// 假值执行，否则跳过。
// $code 为函数体代码，支持首字符特殊指引。
// 目标：当前条目/栈顶1项。
// 注：
...
- **`&`**: 元素属性取值（Property）。
- **`%`**: 元素样式取值。
- **`$`**: tQuery/Collector方法调用。
- **`?`**: 普通对象属性值或方法调用，如 `Date` 成员取值。
- **`^`**: 当前环境成员取值。
- **`!`**: 外部扩展函数库（X）成员引用（不调用）。

> **注：**<br>
> 除对外部函数库的检索引用外，其它目标如果是一个函数，会简单调用取值。

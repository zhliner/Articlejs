...
`Table` 接口仅提供极简单的表格操作，除了表标题的设置和删除，主要是对表主体部分（`<thead>`、`<tbody>`、`<tfoot>`）的表格行执行添加、删除、获取等。所有的操作忽略列数，也即表格的列数是不变的，也不能修改，这极大简化了表格接口的复杂性。而且也不涉及单元格的内容操作，它们可由基础库里的 `.text()` 或 `.html()` 接口实现。

...
在 `node` 之外包裹一个容器元素，如果 `node` 是DOM中的一个节点，该容器元素会替换 `node` 原来的位置。包裹容器可以是一个现有的元素、一个HTML字符串、或一个返回容器元素或HTML字符串的取值函数。
...
被包裹的节点或文本内容，可以是元素（`Element`）、文本节点（`Text`）或内容文本（**注**：`ParentNode.prepend()` 支持文本）。

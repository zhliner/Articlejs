...
tQuery 也支持 Sizzle 中很有用的直接子元素选择器 `>...`。如：`$('>b, >a', ctx)`，表示选取 `ctx` 内 `<b>` 和 `<a>` 直接子元素，但这里的支持并不充分精确。

tQuery 可以脱离 Sizzle 使用，它采用了浏览器内置的 `querySelector()/querySelectorAll()` 通用方法，该方法并不支持上面的选择器形式。tQuery 的实现只是简单地用一个临时属性名补充到 `>` 之前，并不严格检查 `,>` 是否只是属性值的一部分，如：`>[value=",>xx"]`，因此如果采用类似的选择器就无法正常工作。

幸好，或许必须使用这种选择器的场景很少，出于简单性考虑，我们并没有处理这种情况。`,` 和 `>` 必须按顺序在一起才会导致混淆，中间的空格可选。

另外，临时属性名是一个固定的 `___tquery_` 前缀加上当前时间戳（16进制值）的形式，如：`___tquery_b3889cfa_`。在现实中应该很难出现重名冲突（而且即便冲突，也需要在巧合的地方出现才会产生影响）。

这是两个需要注意的地方，虽然它们极难导致问题，但毫无疑问应该让您知道它们。

...
1. **行为定义：**<br>
    ...
    另外，支持一个 `obt-src` 属性导入外部的 `On/By/To` 定义，格式：`{ selector, on, by, to }`。其中 `selector` 为选择器（可选），为测试验证用。
    **注**：如果该配置独立出来，`selector` 就可以成为元素定位标识。
...
evo: {
    ...
    origin: {Element|Collector}     // To：初始检索结果，不被更新的话与 To:updated 相同
}
```
...
这里有一个通用规则：通过模板实参中的 `_` 标识名，表达需要从流程中获取实参。例如：
...
1. 在指令中通过 `_` 标识名从流程中取实参，所取实参为 **该位置及之后全部参数序列**。也即：一个指令调用中只能有一个该标识，且只在最后。
2. 如果需要的实参为多个，用户需要提前打包流程数据为一个数组（`pack`），它们在传递给指令时会自动展开。**注意**：如果只需单个实参但所取值是一个可展开对象（字符串/数组），也需提前打包。


### `obted` 事件

系统提供一个自定义事件：当一个元素/对象的 OBT 构建完成后，向元素发送一个**非冒泡**并**不可取消**的事件 `obted`，这很有用。

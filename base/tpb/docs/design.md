...
这里的设计是：通过模板实参中的 `_` 标识名，表达需要从流程中获取实参。例如：

- `attr('-val')` 表示从流程数据（元素|元素集）上获取 `data-val` 特性值。
- `attr(_)` 表示除了需要从流程数据中获取操作目标（元素|元素集）外，还要从流程中获取取值的特性名（如：`data-val`）。**注意**：`_` 不是字符串（无引号包围）。

从流程中取实参是直接从数据栈上提取的，与暂存区无关（这使得暂存区逻辑不受影响）。

- `evo(1) ev('detail') attr(_)`：先从栈顶取特性名实参（`ev('detail)`），然后再取出事件目标元素（`evo.target`），提取其某特性值。
- `ev('detail') evo(1) pop $(_)`：`pop` 指令提取栈顶1项到暂存区用于 `$(rid)` 的起点元素指定，`_` 从数据栈顶取选择器（`ev('detail)`）执行检索。
- `evo(1) pop ev('detail') $(_)`：效果同上，`pop` 指令先取起点元素到暂存区，`ev('detail)` 入栈数据不影响暂存区。

> **注记：**
> 实参数据先于操作目标取值有一个好处：可适应**实参依赖操作目标**（操作目标在前）和**操作目标依赖实参**（实参在前，通过 `pop` 让位）两种情况。
> 而如果反过来，则只能适应操作目标在后（依赖实参）一种情形了，此时的 `pop` 没有更多的效用。

这种获取动态实参的方式是通用的，基本规则为：

1. 在指令中通过 `_` 标识名从流程中取实参，所取实参对应到 **该位置及之后全部参数序列**。也即：一个指令调用中只能有一个该标识，且只在最后。
2. 通过 `_` 标识取出的实参会自动展开（如果为数组），因此用户可能需要对流程数据提前打包（`pack`）。如果需要单个实参但值本身就是一个数组，应当使用下面的 `_1` 标识。
3. `_` 标识实际上是一个系列，还有 `_1`、`_2`、`_3...9` 等9个固定取值数量标识，分别表示从流程取 `1-9` 个实参。同样地，它们也位于模板实参的最后。

> **注：**<br>
> 之所以设计这9个标识，一是它们可以清楚地表意，二也是可以省掉 `pack` 封装指令，且实现简单。<br>
> `_` （同 `_0`）实际上只是取栈顶1项展开后传递，适用不定数量参数的情况。如果取值目标不是数组，`_` 和 `_1` 效果一样。<br>

...
这有利于简化模板中重复的 OBT 配置：重复的配置可以预定义在**仅一个**元素上（`@somekey|...`），其它后续元素简单的配置为 `on="^obted|bind('somekey')"` 即可。
...
// 有值则为相等（===）比较，否则为目标真值测试。
// 注意：
// - 不自动取栈，因此单纯的exit实际上是 undefined === undefined 为真，结束流程。
...
// 如果当前条目非空，以当前条目为合并目标（实参非空）或展开合并当前条目成员（.flat(1)）。

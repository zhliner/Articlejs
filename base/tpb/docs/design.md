...
从流程中取实参是直接从数据栈上提取的，与暂存区无关（这使得暂存区逻辑不受影响）。如：

- `ev('detail') evo(1) attr(_)`：先取出事件目标元素（`evo.target`），然后再从栈顶取特性名实参（`ev('detail)`）。

这种获取动态实参的方式是通用的，基本规则为：

1. 在指令中通过 `_` 标识名从流程中取实参，所取实参为 **该位置及之后全部参数序列**。也即：一个指令调用中只能有一个该标识，且只在最后。
2. 如果需要的实参为多个，用户需要提前打包流程数据为一个数组（`pack`），它们在传递给指令时会自动展开。如果只需单个实参但所取值本身就是一个数组，也需提前打包（二维封装）。

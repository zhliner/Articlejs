...
2. 通过 `_` 标识取出的实参会自动展开（如果为数组），因此用户可能需要提前打包（`pack`）流程数据为单一数组。如果只需单个实参但所取值本身就是一个数组，使用下面的 `_1` 标识会更简单。
...
> `_` （同 `_0`）只是取栈顶1项展开后传递到指令后续实参位置，适用不定数量参数的情况（比如从 `Event.detail` 取值）。<br>

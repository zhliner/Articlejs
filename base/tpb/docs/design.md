...
- `attribute('-val')` 表示从流程数据（元素|元素集）上获取 `data-val` 特性值。
- `attribute(_)` 表示除了需要从流程数据中获取操作目标（元素|元素集）外，还要从流程中获取取值的特性名（如：`data-val`）。**注意**：`_` 不是字符串（无引号包围）。

为了从流程中同时获取操作目标和取值的特性名，流程数据需要提前打包（`pack`），以包含特性名称。如：

- `evo(1) ev('detail') attribute(_)`。
- 参考：`evo(1) attribute('-val')` 为直接取值指令序列，特性名固定不变。

其中操作目标是事件起点元素（`evo(1)`），特性名从发送的事件的 `detail` 属性上捕获，`attribute(_)` 从栈顶先取出实参（特性名），然后再按正常的逻辑操作目标（即起点元素）。

这种从流程中获取动态实参的方式是通用的，基本规则是：

1. 在指令中通过 `_` 标识名表示从流程中取实参，具体含义是：**该位置以及之后全部的实参序列从流程数据中提取**。也即：一个指令调用中只能有一个此标识，且位于最后。
2. 用户可能需要在该指令之前打包流程数据（结果为一个数组）来作为待取用的实参序列，除非实参只是单个且本身不是数组。**注**：系统会自动提取1项数据展开添加到模板实参序列之后。
3. 该动态实参会先于指令的操作目标被提取。如果暂存区无值，数据栈的顶部1项数据会被取出，如果暂存区有值，则是暂存区首项被取出。之后为普通的指令操作取项。

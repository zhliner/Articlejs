...
系统提供一个自定义事件，即：当一个元素/对象的 OBT 构建完成后，向元素发送一个**非冒泡**并**不可取消**的事件 `obted`。这使得模板中重复的 OBT 配置可以设计为共享方式：重复的配置预定义在**仅一个**元素上（`@evname|...`），其它后续元素简单的配置为 `on="^obted|$(rid)" to="|bind('evname')"` 即可。

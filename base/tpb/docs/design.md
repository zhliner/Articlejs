...
- `evo(1) ev('detail') pack(2) attribute(_)`。
- 参考：`evo(1) attribute('-val')` 为直接取值指令序列，特性名固定不变。

其中操作目标是事件起点元素（`evo(1)`），特性名从发送的事件的 `detail` 属性上捕获，`pack(2)` 则将这两项数据打包为一个单一数组（2个成员）。`attribute(_)` 则从这个单一数组上提取目标（首个成员）和特性名（第二个成员）。

这种从流程中获取动态实参的方式是通用的，基本规则是：

1. 在指令中通过 `_` 标识名表示从流程中取实参，一个指令调用中只能有一个此标识，表示：**从流程数据中获取该位置以及之后全部位置的实参序列**。
2. 用户通常需要在该指令之前打包流程数据（结果为数组），包含指令的操作目标和待取用的实参。实参可以有多个，他们并列在目标之后。**注**：无目标且单实参且实参非数组时可无需打包。
3. 不同指令的取值项数（`[EXTENT]`）可能不同，大于1（n>1）的指令的操作目标为该结果数组的前n个成员，0和1取值项数指令的操作目标是首个成员，无取值项数（`null`）指令没有操作目标，全为实参序列。
4. 特权指令可能自行从暂存区/数据栈取值，但这些取值晚于实参从流程数据提取。因此并不影响实参序列。

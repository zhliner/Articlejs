...
- 无实参传递的指令可省略括号。
- 前置 `^` 的事件名表示单次执行绑定（`$.one`）。
- 预存储调用链的事件名前置 `@` 字符，名称之后的冒号 `:` 用于分隔标识ID（可选，不含有空白）。

> **注：**<br>
> 预存储的调用链事件名后需要包含ID标识，是因为相同的事件处理可以多次绑定到同一个元素上，所以需要区分。<br>
> 这个ID值是任意的（字母或数字，由 `To` 部分的 `bind/once` 指令使用），如果一个元素不用同时绑定多个相同的事件名/预存储调用链，它就是可选的。<br>

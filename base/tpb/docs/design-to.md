...
- `[]` 指定下标范围或特定的位置，格式：`(...)[beg:end]` 或 `(...)[x, y, z]`。冒号分隔起止范围，逗号分隔特定位置。**注**：两者不可混用。
- `{}` 用一个表达式实现过滤，接口：`(e:Element, n:Number, c:Collector): Boolean`，参数名固定，表达式无需 `return`。如：`(...){ n%2 && n<=10}`，返回集合中前5个偶数位置成员。

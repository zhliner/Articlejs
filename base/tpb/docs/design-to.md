...
- `()` 指定范围的起止下标，格式：`(...)(beg, end)`，不含末尾下标成员。如：`(#xyz b)(1, 3)`，检索id为 `xyz` 元素的 `<b>` 子元素，只取其中 `第2` 和 `第3` 两个成员。
- `[]` 指定单独的位置下标，格式：`(...)[x, y, ...]`。如：`(#abc > b)[0,2,4]`，检索id为 `abc` 元素的直接子 `<b>` 元素，只取第 `1、3、5` 号成员。
- `{}` 用一个表达式实现过滤，接口：`(v:Element, i:Number, o:Collector): Boolean`，参数名固定，表达式无需 `return`。如：`(...){i%2}`，返回集合中偶数位置的成员。
...
**另外**：短横线（`-`）是一个占位符，因为起点元素默认就是事件委托元素，所以它实际上也可以指代委托元素（`elo.delegate`）。通常情况下，该占位符无需书写，如 `-|append` 与 `|append` 等同。**注**：作为一种编码习惯，如果前方指令指定了起点元素（`pop`），则此处可前置 `-` 字符以示区别（仅友好提示），如：`<p on="click|... pop" to="-|append">`，此处的 `to` 段检索起点由前阶 `pop` 指定。
...
(xxx){filter}
// 回调过滤：Collector
// {} 内为过滤表达式，参数名固定：(v:Element, i:Number, o:Collector): Boolean。
// 注：即 filter 的逻辑。
// 例：

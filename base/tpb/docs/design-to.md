...
> **另外：**<br>
> 短横线（`-`）是一个占位符，因为起点元素默认就是事件委托元素，所以它也可以表达委托元素（`elo.delegate`）。通常情况下，该占位符无需书写，如 `-|append` 与 `|append` 等同。<br>
> 作为一种编码习惯，如果前方指令指定了起点元素（`pop`），则此处可前置 `-` 字符以示区别（仅友好提示），如：`<p on="click|... pop" to="-|append">`，此处的 `to` 段检索起点由前阶 `pop` 指定。<br>

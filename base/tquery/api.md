...
- `idx: Number` 目标成员的位置下标，支持负数从末尾算起。假值视为 `0` 下标，兼容字符串数字。

下标位置超出集合范围或非法时构造为一个空的集合。
...
- `fmt: String` 待切分的格式串，**字符串**内的分隔符不会被视为分隔符。
- `sep: String` 分隔符。仅支持单个字符，但4字节的Unicode字符也有效。
- `cnt: Number` 切分的最大次数（如：1次分为2片，2次分为3片）。可选，默认全部切分。

这在字符串本身是某种格式串时有用，比如对于选择器串：`>b, >[value="x, >yz"]`，按逗号（`,`）切分，但不能将属性值 `x, >yz` 也切分开。这时就需要切分函数识别格式串内的**字符串**结构了。
这也是直接子元素选择器 `>...` 的内部切分处理函数。

> **注：**<br>
> 该切分函数仅识别**字符串**（单/双引号和撇号）结构，不识别其它如属性（`[...]`）结构。

```js
[...$.splitf('>b, >[value="x, >yz"],label', ',')];
// [
//     '>b',
//     ' >[value="x, >yz"]',
//     'label'

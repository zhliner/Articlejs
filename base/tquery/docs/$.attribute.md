# $.attribute( el, names, value ): Value | [Value] | Element
...
- 取值时：为字符串，单个名称或空格分隔的多个名称序列。单个名称时返回单个值，多个名称时返回一个值集。
...
取值时返回一个值（单个特性时）或一个值集（多个特性时）。设置时返回操作的目标元素自身（即实参 `el`）。
...
**取值时：**
- `names` 支持字符串数组，分别与集合成员一一对应取值。无对应项的元素简单忽略。

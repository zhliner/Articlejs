# $.attribute( el, names, value ): Value | Object | Element
...
- 取值时：为字符串，单个名称或空格分隔的多个名称序列。单个名称时返回单个值，多个名称时返回一个 **名:值** 对象，其中名称维持原始传入的实参形式。
...
取值时返回一个值（单个特性时）或一个对象（多个特性时）。设置时返回操作的目标元素自身（即实参 `el`）。


## 轻量版 $.attr( el, name, value ): Value | Element

参数 `name` 仅支持单个名称，且不支持对象类型用于设置，其它参考 `$.attribute` 接口说明。

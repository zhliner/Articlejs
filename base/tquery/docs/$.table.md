...
### .body( idx, rows, tsec? ): [Element] | Collector

在 `tsec` 内添加表格行，新插入的表格行包含固定数量（初始列数）的空单元格。`tsec` 为 `<tbody>` 元素，可选，默认为 `<table>` 内首个 `<tbody>`。
...
- `idx: Number` 新行插入的位置。`-1` 表示插入到 `tsec` 内末尾。
- `rows: Number` 新插入的行数。
- `tsec: Element` 插入到的目标容器，可选，默认为表格内首个 `<tbody>`。
...
### .get( idx, tsec? ): Element

获取目标位置的行元素，表格行计数规则同上。
...
### .gets( idx, size, tsec? ): Collector

获取目标位置及之后的行元素集，表格行计数针对 `tsec`，未指定 `tsec` 时针对整个表格。
...
### .removes( idx, size, tsec? ): Collector
...
### .insertColumn( idx ): [Element]

插入一列单元格。新的单元格类型（`<td>` 或 `<th>`）与参考单元格相同。是向前插入，新的列会占据原参考列的位置。


#### 参数

- `idx: Number` 位置下标（从 `0` 开始），支持 `-1` 插入到末尾。
...
### .insertColumns( idx, cols ): Collector
...
- `idx: Number` 位置下标（从 `0` 开始），支持 `-1` 插入到末尾。
- `cols: Number` 新插入的列数。若未传入值则默认插入1列。
...
- `idx: Number` 位置下标（从 `0` 开始），支持负值从末尾算起。
- `cols: Number` 将要删除的列数。若未传入值则默认删除目标位置及之后全部。


#### 返回值

被删除的行单元格段（连续单元格数组）的集合（`Collector`）。

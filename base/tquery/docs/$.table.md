...
**注意**：如果传递 `true` 则要求行数必须大于零（至少包含一行）。否则无效果。
...
构造函数，封装一个表格或创建包含一个新表格的 `Table` 实例。这只是一个简单的封装或创建，用户需要调用 `.build(...)` 构建之后才能正常使用。


#### 参数

- `tbl: Element` 待封装的表格元素（`<table>`），可选。如果没有已有的表格传入，则内部会新建一个空的 `<table>`（不含任何子单元）。
...
### .build( cols, rows ): Number | void

用指定的列数和行数构建 `Table` 实例。

如果新建时传入了一个已有的表格（`<table>`）且表格包含有表格行（`<tr>`），则无需传递任何实参，初始化会解析已有的表格列数并设置为内部属性。如果新建时没有传入表格，或者已有的表格没有数据（`<tr>`），则需传递行列数进行构建。这通常是一种初始化动作（除非内部表格被清空了子单元）。

传入行列数初始化时，内部的 `<table>` 元素会包含一个 `<tbody>` 子结构（不含 `<thead>` 或 `<tfoot>`）。

零行的表格是有效的，但列数必须大于零，否则没有意义。
...
如果是初始化解析一个已有的表格，返回该表格的列数，否则无返回值。

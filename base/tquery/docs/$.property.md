# $.property( el, names, value ): Value | Object | Element
...
- 支持一个定制的 `selected` 属性名，但仅适用 `<select>` 元素，返回选取的 `<option>` 元素或 `null`（无任何选取）或一个空数组（对于多选选单）。
- 支持一个定制的 `checkedNode` 属性名，适用单选和复选按钮，返回选中的按钮控件元素。但复选按钮时会返回一个数组，不论选中的元素是单个或是多个。未选中时返回 `null` 或一个空数组。

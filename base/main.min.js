...
import e,{XLoader as t,TplsPool as n,OBTA as r,TplrName as l,Web as o,DEBUG as i}from"./tpb/config.js";import s from"./shortcuts.js";import{processProxy as a,processExtend as u,customGetter as c,obtBuilder as f,BaseOn as d,BaseBy as p,Tpb as h}from"./tpb/tpb.min.js";import{HotKey as m,ObjKey as g}from"./tpb/tools/hotkey.js";import{setupRoot as _}from"../index.js";import{Scripter as w,By as y,Sys as v,Limit as b,On as x,Local as E,Cmdx as T,Tips as S,Help as C}from"../config.js";import{Render as N}from"./tpb/tools/render.js";import{Hicolor as A}from"./highlight/main.js";import{Spliter as $,UmpCaller as k,UmpChars as O,UmpString as R}from"./tpb/tools/spliter.js";import{Util as I}from"./tpb/tools/util.js";import{Templater as L}from"./tpb/tools/templater.js";
...
window.GHK=(new m).config(s.Global),window.SHK=(new m).config(s.Slave),window.DHK=(new m).config(s.Modal),window.CHK=new g(xs).config(s.Content),window.Sys=v,window.Kit=Es,window.KM=s.Keys,window.edInit=function(t,r,o,i,s,a,u,c,f){xo=e.get(t),Eo=e.get(r),ko=e.get(o),To=e.get(i),So=e.get(s),Co=e.get(a),No=e.get(u),Ao=e.get(c),$o=e.get(f),Object.assign(ts,{[T.select]:new $l(lo,xo),[T.filter]:new kl(lo),[T.search]:new Ol(xo),[T.command]:new Rl,[T.calcuate]:new Il(lo,xo)}),Fl(e.get("#x-help"),n.get(l).node(v.modalPlug)),e.on(xo,"attrdone styledone nodesdone emptied detach normalize propdone",null,so)},window.Api=Ts,window.ESC=new xn,e.config({varyevent:!0}),h.Init(x,y),h.build(document,o.tplmap).then((()=>e.trigger(document.body,"finish"))).then((()=>Ts.init("#outline","#editor","#content","#help","#beeptip"))).then((()=>v.readyCall())).catch((e=>v.failCall(e))),i&&(window.On=x,window.By=y,window.Tpls=n.get(l));
